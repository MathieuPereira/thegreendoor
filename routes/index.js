...
router.get('/home', async function (req, res, next) {
    ...
    if (req.query.date === 'to-be-started') {
...
router.get('/show-sale', async function (req, res, next) {
    ...
    if (!sale) {
        ...
        if (req.query.productName != null && req.query.productName !== "undefined") {
            ...
            res.status(200).json({
                saleLabels: sale.brandLabels,
                saleImg: sale.brandImg,
                saleEnding: sale.endingDate,
                products: products,
            });
        }
    }
...
router.post('/create-checkout-session', async (req, res) => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        cancel_url: 'https://thegreendoor.herokuapp.com//basket',
    });
